...
function set_point_cloud_data(position2, color, geometry) {
  geometry.setAttribute("position", new THREE.Float32BufferAttribute(position2.flat(), 3));
  if (color === null) {
...
export default {
  ...
  mounted() {
    ...
    this.scene.add(new THREE.AmbientLight(16777215, 0.7 * Math.PI));
    const light = new THREE.DirectionalLight(16777215, 0.3 * Math.PI);
    ...
    create(type, id, parent_id, ...args) {
      ...
      if (type == "group") {
        ...
        if (type == "stl") {
          ...
          this.stl_loader.load(url, (geometry2) => mesh.geometry = geometry2);
        }
        let material;
        if (wireframe) {
