...
export default {
  ...
  async mounted() {
    await this.$nextTick();
    await loadResource(window.path_prefix + "".concat(this.resourcePath, "/leaflet/leaflet.css"));
    window.L = L;
    await Promise.all(this.additionalResources.map((resource) => loadResource(resource)));
    if (this.drawControl) {
      await Promise.all([
        loadResource(window.path_prefix + "".concat(this.resourcePath, "/leaflet-draw/leaflet.draw.css")),
        loadResource(window.path_prefix + "".concat(this.resourcePath, "/leaflet-draw/leaflet.draw.js"))
      ]);
    }
