  ...
  methods: {
    ...
    add_style(style) {
      Object.entries(style).forEach(
        ([key, val]) => document.querySelectorAll(this.selector).forEach((e) => e.style.setProperty(key, val))
      );
    },
    remove_style(keys) {
    ...
    add_props(props) {
      Object.entries(props).forEach(
        ([key, val]) => document.querySelectorAll(this.selector).forEach((e) => e.setAttribute(key, val))
      );
    },
    remove_props(keys) {
