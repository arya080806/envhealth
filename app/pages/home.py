...
@media (min-width: 900px) and (orientation: landscape) {
    .home-page {
        ...
        border-left: 1px solid rgba(244,240,230,0.08);
        border-right: 1px solid rgba(244,240,230,0.08);
    }
        ...
        with ui.element('div').classes('home-page'):
            with ui.element('div').classes('home-shell'):
                with ui.element('div').classes('home-top'):
                    ui.html('<div class="home-mark">INSPIRATION WORLD</div>', sanitize=False)

                with ui.element('div').classes('home-copy'):
                    ui.html(
                        '''
                        <h1 class="home-title">灵感世界</h1>
                        <div class="home-title-en">Inspiration World</div>
                        ''',
                        sanitize=False,
                    )
                ...
                with ui.element('div').classes('home-panel animate-in'):
                    ui.html('<div class="home-panel-title">Start a session</div>', sanitize=False)
                    with ui.element('button').classes('home-start').on(
                        'click', lambda: smooth_navigate('/camera')
                    ):
                        ui.html('开始体验')
